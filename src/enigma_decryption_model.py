...
class EnigmaDecryptionModel:
    ...
    def train_two(self, input_tensor, target_tensor):
        ...
        lengths = self.get_lengths(target_tensor)
        ...
        if use_teacher_forcing:
            for t in range(settings.MAX_SEQUENCE_LENGTH):
                ...
                mask_loss, nTotal = self.maskNLLLoss(decoder_output, target_tensor[t].unsqueeze(1), mask)
                loss += mask_loss
            ...
            for t in range(settings.MAX_SEQUENCE_LENGTH):
                ...
                mask_loss, nTotal = self.maskNLLLoss(decoder_output, target_tensor[t], mask)
                loss += mask_loss
                print_losses.append(mask_loss.item() * nTotal)
                n_totals += nTotal
    ...
    def get_mask(self, tensor: torch.Tensor) -> torch.Tensor:
        return (tensor != self.cipher_database.pad_token_index).to(self.device)
